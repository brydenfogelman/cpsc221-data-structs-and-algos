...
bool delete_node(Node*& root, int key) {
  ...
  // case 3: target has two children
     ...
     */
	Node * n = target->left;
	// if right is null then n is left node
	while(n->right != NULL) {
		n = n->right;
	}
...
int numNodes( Node* root ) {
   ...
   */
  if(root != NULL) return 1 + numNodes(root->right) + numNodes(root->left);
  return 0;
}
...
int height( Node* x ) {
   ...
   */
   ...
   //if(x->left != NULL) return 1 + height(x->left);
   //if(x->right != NULL) return 1 + height(x->right);
   //if(x->left != NULL) return 1 + height(x->left);
...
int depth( Node* root, Node* x ) {
   ...
   * THIS FUNCTION NEEDS TO BE IMPLEMENTED ********************
   */
  if(root == NULL || root == x) return 0;
  if(root->key > x->key) return 1 + depth(root->left,x);
...
void in_order( Node* rootNode, int level, Visitor& v ) {
   ...
   * THIS FUNCTION NEEDS TO BE IMPLEMENTED ********************
   */
	if(rootNode->left != NULL) in_order(rootNode->left, level, v);
	v.visit(rootNode, level);
	if(rootNode->right != NULL) in_order(rootNode->right, level, v);
}
...
void pre_order( Node* rootNode, int level, Visitor& v ) {
   ...
   */
	v.visit(rootNode, level);
	if(rootNode->left != NULL) pre_order(rootNode->left, level, v);
	if(rootNode->right != NULL) pre_order(rootNode->right, level, v);
}
...
void post_order( Node* rootNode, int level, Visitor& v ) {
   ...
   * THIS FUNCTION NEEDS TO BE IMPLEMENTED ********************
   */
	if(rootNode->left != NULL) post_order(rootNode->left, level, v);
	if(rootNode->right != NULL) post_order(rootNode->right, level, v);
	v.visit(rootNode, level);
}
