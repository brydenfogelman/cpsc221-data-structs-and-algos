...
int numNodes( Node* root ) {
  ...
  if(root->right) totalSum+= numNodes(root->right);
  if(root->left) totalSum+= numNodes(root->left);
...
int numLeaves( Node* root ) {
  ...
  if(root->right) totalSum+= numLeaves(root->right);
  if(root->left) totalSum+= numLeaves(root->left);
...
int depth( Node* root, Node* x ) {
  ...
  if(root->left){
     depthVar = depth(root->left, x);
     depthVar++;
  }
  if(root->right){
    depthVar = depth(root->right, x);
    depthVar++;
  }
  return depthVar;
